...
class TestParallelExecution(unittest.TestCase):
    ...
    def test_parallel_case(self):
        ...
        self.assertEqual(result[0]["startTime"], "08:00")

        self.assertEqual(result[1]["sampleId"], "S2")
        self.assertEqual(result[1]["technicianId"], "T001")
        self.assertEqual(result[1]["equipmentId"], "E001")
        self.assertEqual(result[1]["startTime"], "08:10")

        self.assertEqual(result[2]["sampleId"], "S3")
        self.assertEqual(result[2]["technicianId"], "T002")
        self.assertEqual(result[2]["equipmentId"], "E001")
        self.assertEqual(result[2]["startTime"], "08:20")

    def test_parallel_case_2(self):
        samples = [
            Sample("S1", "BLOOD", "STAT", 30, "08:00", "P1"),
            Sample("S2", "BLOOD", "STAT", 20, "08:10", "P2"),
        ]
        technicians = [
            Technician("T001", "TECH01", "BLOOD", "08:00", "18:00", 1.0),
        ...
        equipments = [Equipment("E001", "EQUI01", "BLOOD", True, 2, 1)]
        schedule = Scheduler(samples, technicians, equipments)
        schedule.planify()
        result = schedule.get_schedule()
